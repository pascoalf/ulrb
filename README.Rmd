...
used to divide species within a sample into clusters. The clusters are then
ordered based on a user-defined classification vector. By default, our 
method classifies all species in: "rare", "undetermined" or "abundant". In alternative, 
the user can change the number of classifications. To do so, ulrb includes functions to help
the user decide the number of classifications (k), but it is also possible for ulrb to 
automatically decide the number of clusters. Besides clustering, uleb includes functions to 
...
**Note: the article to cite ulrb is undergoind peer-review in Communications Biology journal**
