...
It is good practice to cehck if results from unsupervised learning look sane. To help with this task, we made a group of functions that will help researchers understand if they need more or less classifications, or if the algorithm just doesn't work. There is no hard rule to verify unsupervised learning, but results should look sensible and follow expectations. We have seen that the results from this method are consistent with previous definitions of rarity, for example, most taxonomic units are rare and only very few are abundant, but we avoid introducing a human decision on which points are rare or abundant. 

The sanity checks are the (1) Rank Abundance Curve and the (2) Silhouette plots. The first will be more helpfull for microbial ecologists, while the second will be more helpfull for data scientists.

### (1) Rank Abunddance Curve (RAC)

The RAC is a common tool in microbial ecology to describe the detectation limits of different methods and the distribution of species abundance (Pedrós-Alió, 2012). The RAC described for most environments shows that a few microorganisms are very abundant, but most follow the "long-tail", which is called the rare biosphere. But the point where the "long-tail" begins is subjective.

The ulrb method considers that the terms "rare" and "abundant" are relative, i.e. one taxonomic unit is rare if, and only if, compared to an abundant one. We explore the consequences and assumptions that play into this in REF. But this means that the method is not looking for the begining of the "long-tail", it is comparing the abundance of all taxonomic units, and clustering them accordingly.

Thus, if we plot the standard RAC and then collor each taxonomic unit according with the unsupervised classification, we can check if the unsupervised classification is consistent with the notion that the rare-biosphere is within the "long-tail".
...
From the RAC obtained, we confirm that the unsupervised classification provides a sensible result. Because the rare biosphere is indeed in the "long-tail", followed by a transition region, which we term "undertermined", because they are neither rare, nore abundant, and a few extreme points which are abundant.
...
Finally, we provide the function sanity_check_all that wraps around sanity_check_sample_rb and sanity_check_sample_sil to plot the RAC and Silhouette plot of each sample, side by side.
