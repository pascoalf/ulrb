...
So far, we have been considering that the microbial communities are divided into "rare", "undetermined" and "abundant". This division implies that the partition around medoids algorithm is considering **k=3**, *i.e.* there are three clusters. However, there are at least two situations where you might want to change the number of clusters:
...
you can chose to use a grid, or plot all samples; but this wont work for a high number of samples. With a centrality metric, however, we are able to see any number of samples in a single plot.
...
We can do exactly the same thing we did before, but with only "rare" and "abundant" classifications, to do that we set k = 2. Note that we do not explicitly set k = 2 in the `define_rb()` function, instead, we specify our **classification vector**.
...
We do not recommend using k = 2, because it implies that there is a hard distinction between rare and abundant taxa. We consider that there should always be an intermediate or undetermined group, otherwise, we will always have species with very similar abundance scores, but opposing classifications, which is misleading. For more details on this reasoning, see Pascoal et al., 2023.
...
rb_k4 <- define_rb(nice_tidy, 
                   classification_vector = c("very rare", "rare", 
...
plot_ulrb(rb_k5,
          sample_id = "ERR2044662", 
          taxa_col = "OTU",
          colors = c("#009E73", "#F0E442", "grey", "#D55E00", "#CC79A7"),
          log_scaled = TRUE)
```
...
Each of these metrics evaluates different aspects of one clustering result; thus, we can calculate them for
the clustering obtained by each value of k. By comparing the values obtained for each k, you can select the k that got best score. To do this, consider the following:
...
The most straight forward way to evaluate a sensible range of k's is by using the function `suggest_k()`.
This function includes an argument for detailed results, which will give you the results for the three indices, between k = 3 and k = 10. We don't recommend testing k's outside of this range for the purpose of
defining the rare biosphere, but you can analyse all possible k's (just change the range argument).
...
In the last plot, each value of k represents a clustering result. From the above information, we would select k = 3 (just like the default option), because it had the highest average Silhouette score.

We can repeate for all other indices:
...
Thus, for sample 1 (ERR2044662), the best k would be:
 ...
 - 10 based on Calinsky-Harabasz index.
...
We can decide what k to use based on any of this metrics, beeing aware that they measure different aspects of the clustering results. The function `suggest_k()`, by default, will:
...
Instead of the average Silhouette score, you can select another index (Davies-Bouldin or Calinsky-Harabasz), and the function `suggest_k()` will return the best k based on that index.
...
(if you try to run this code, it might take a while)
