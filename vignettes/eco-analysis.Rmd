...
nice_diversity_tidy <- nice_diversity %>% 
  pivot_longer(cols = c("SpeciesRichness", "ShannonIndex", "SimpsonIndex"),
               names_to = "Index",
               values_to = "Diversity") %>% 
  mutate(Month = factor(Month, levels = c("March", "April", "June"))) ## correct order
```
...
Note: the code will be a little bit repetitive, in the future I might improve it.

```{r, fig.height = 8, fig.width= 8}
rare_biosphere_water <- 
  nice_eco %>%  
  filter(Classification == "Rare") %>% 
  select(Sample, Abundance, OTU, Depth, Month, Water.mass) %>% 
  mutate(Sample_unique = paste(Sample, Water.mass))

rb_sp_prep_water <- rare_biosphere_water %>% 
  ungroup() %>% 
  select(Sample_unique, Abundance, OTU)
...
rb_sp_water <- 
  ...
  unchop(everything())
#
rb_sp_water[is.na(rb_sp_water)] <- 0

#
rb_env_water <- rare_biosphere_water %>% 
  ungroup() %>% 
  select(Sample_unique, Depth, Water.mass, Month) %>% 
  distinct()
...
#
rb_env_water %>% 
  mutate(col_water = case_when(Water.mass == "PSW" ~ qualitative_colors[2],
                               Water.mass == "PSWw" ~ qualitative_colors[5],
                               Water.mass == "MAW" ~ qualitative_colors[3],
                               Water.mass == "AW" ~ qualitative_colors[1])) %>% 
  mutate()

#
cca_plot_rare_biosphere_water <- 
  cca(rb_sp_water[,-1], display = "sites", scale = TRUE) %>% 
  plot(display = "sites", type = "p", main = "Rare biosphere")

#
cca_plot_rare_biosphere_water
points(cca_plot_rare_biosphere_water,
       bg = rb_env_water$col_water,
       pch = 21, 
       col = "grey", 
       cex = 2)
#
with(rb_env,
     ordispider(cca_plot_rare_biosphere_water,
                Water.mass, lty = "dashed", label = TRUE))
```

From the last plot, it seems that the rare biosphere is more distinct in the Modified Atlantic Water.

# Final considerations

Hopefully, you got a sense of how the ulrb package can be integrated in a more general microbial ecology workflow. Of course, many more questions could be approaced, such as taxonomy and so on. 
